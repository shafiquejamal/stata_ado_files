program pmt_svy, eclass
	...
	foreach x of numlist `cutoffs' { // loop over all the cutoffs to be used
		...
		forv q = 1/`quantiles' {
			...
			ereturn scalar coverage_cutoff`x'_quantile`q' = 	r(mean)
		...
		ereturn scalar leakage_`x' = `leakage'
		ereturn scalar undercoverage_`x' = `undercoverage'
	}	
end program
