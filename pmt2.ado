program define pmt2, rclass
	...
	syntax varlist(min=2 ts) [if] [pw aw iw fw],  Poor(varname numeric) quantilec(varname numeric) [Cutoffs(numlist asc min=1 max=20 >0 <=70 integer) CABsolute(numlist asc min=0 max=20 >=0) Graphme(integer -1) logpline(real 0) Usesubsamplebetas(varname numeric) Filter GENerate(name)]
	version 9.1 
	marksample touse
